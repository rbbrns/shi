		...
		if args:
			...
			try:
				...
				if "dprint(" in source_line:
					...
					var_names = [arg.strip() for arg in args_str.split(",") if arg.strip()]
				else:
	...
	if stack:
		...
		for i, (filename, line_num, func_name, args_dict) in enumerate(reversed(stack)):
			...
			if func_name is None:
				...
				if args_dict:
					args_str = ", ".join([f"{Fore.GREEN}{k}{Style.RESET_ALL}={_format_value(v, max_length=30)}" 
...
def _print_variable(name: str, type_name: str, value_repr: str, depth: int = 0):
	...
	indent = "   " * (depth+1)
	print(f"{indent}{Fore.GREEN}{name}{Style.RESET_ALL}: {Fore.BLUE}{type_name}{Style.RESET_ALL} = {value_repr}")
