...
def take_order(
    ...
    info = (
        get_customer_info()
    )  # Resolves customer_name, table_number from parent context
...
def calculate_item_price(
    menu_item, quantity, base_price, TAX_RATE, SERVICE_CHARGE_RATE
):
    """
    ...
    print(
        f"  {menu_item} ({quantity}x): Base ${base_price:.2f}, Total (incl. tax/service) ${final_price:.2f}"
    )
    return final_price
